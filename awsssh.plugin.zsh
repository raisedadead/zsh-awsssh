...
# Launch connections and move them to asw_ssh session with better window names
# Launch connections and move them to asw_ssh session with better window names
_launch_connections() {
  local selections="$1"
  local connection="$2"
  ...
  while IFS= read -r selection; do
    local name=$(echo $selection | awk '{print $1}')
    local instance_id=$(echo $selection | awk '{print $2}')
    local session_name="aws_ssh_${name}_${instance_id}"
    local window_name="${name}_${instance_id}" # Define a meaningful window name
    if ! tmux has-session -t "$session_name" 2>/dev/null; then
      # Create the session without attaching
      tmux new-session -d -s "$session_name" "zsh -c 'source $HOME/.zshrc; _aws_ssh_command \"$selection\" \"$connection\" \"$username\"; zsh'"

      # Move the session to asw_ssh as a new window
      tmux rename-session -t "$session_name" temp_name_for_moving_$session_name # Temp rename to avoid name conflict
      tmux link-window -s "temp_name_for_moving_$session_name" -t "asw_ssh"
      tmux kill-session -t "temp_name_for_moving_$session_name" # Cleanup temp session

      # Rename the newly created window to something meaningful
      tmux rename-window -t "asw_ssh:${window_name}" "$window_name"
    fi
  done <<<"$selections"
